...
class MultiWozEvaluator(object):
    ...
    def _eval_dial_with_more_return_info(self, dialog, goal, real_requestables, counts,
                                                          soft_acc=False, same_eval_as_cambridge=False):
        """Evaluates the dialogue created by the model.
            First we load the user goal of the dialogue, then for each turn
            generated by the system we look for key-words.
        ...
        return success, match, stats, counts, venue_offered, provided_requestables
...
if __name__ == '__main__':
    evaluator = MultiWozEvaluator(MultiWozReader(GPT2Tokenizer.from_pretrained(cfg.gpt_path)))
    dial_dict = json.load(open('./result/e2e_gen_result18_20_34.json','r'))
    test_dial_id = 'sng01679'
    dials = {test_dial_id:dial_dict[test_dial_id],'mul0842':dial_dict['mul0842']}
    for id in dials.keys():
        dials[id] = dials[id][:-1]
    ...
    for dial_id in dials:
        ...
        counts = 0
        # print('\n',dial_id)
        success, match, stats, counts = evaluator._evaluateGeneratedDialogue(dial, goal, reqs, counts,
                                                                        same_eval_as_cambridge=cfg.same_eval_as_cambridge)
        print('end')
